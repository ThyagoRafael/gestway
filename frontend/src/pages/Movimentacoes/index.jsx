...
import {
	FiSearch, FiChevronDown, FiRefreshCw, FiCheck,
...
export default function Movimentacoes() {
	...
	const [tiposAtivos,  setTiposAtivos]  = useState([...TIPOS]);
	...
	// filtro de período
	const diasPeriodo = PERIODOS[periodoIdx].dias;
	let dados = todas.filter((m) => {
		if (diasPeriodo !== null) {
			// dataHora vem formatada "DD/MM/YYYY - HH:MM", precisamos comparar
			// como não temos a ISO aqui, filtramos só pelos tipos por enquanto
		}
		return Object.values(m).join(" ").toLowerCase().includes(busca.toLowerCase());
	});

	if (tiposAtivos.length < TIPOS.length)
		dados = dados.filter((m) => tiposAtivos.includes(m.tipo));
	...
	const handleAtualizar = () => { setPagina(1); carregar(); };
		...
		<div className={styles.wrapper}>
			<main className={styles.page}>
							...
							<div className={styles.dropMenu}>
								{TIPOS.map((t) => (
									<label key={t} className={styles.tipoItem}>
										<input type="checkbox" checked={tiposAtivos.includes(t)} onChange={() => { toggleTipo(t); setPagina(1); }} />
					...
					<table className={styles.table}>
						...
						<tbody>
							{carregando ? (
								...
								<tr><td colSpan={6} className={styles.vazio}>{erroApi}</td></tr>
							) : dadosPag.length === 0 ? (
				...
				<div className={styles.rodape}>
					<span className={styles.total}>{dados.length} de {todas.length} resultado(s)</span>
