...
import {
	...
	FiPhone, FiMail, FiInstagram, FiChevronRight
} from "react-icons/fi";
...
export default function LandingPage() {
	...
	const { banner1, banner2, grid1, grid2, grid3, exibirBanner,
	...
	const prodGrid3 = (grid3?.slots ?? []).filter(Boolean);

	// busca filtra produtos dos dois grids
	const termoBusca = busca.trim().toLowerCase();
	const buscando = termoBusca.length > 0;
	const resultadosBusca = useMemo(() => {
		if (!buscando) return [];
		return [...prodGrid1, ...prodGrid2].filter(p =>
			p.nome.toLowerCase().includes(termoBusca)
		);
	}, [termoBusca, buscando, prodGrid1, prodGrid2]);
			...
			<header className={styles.header}>
				<div className={styles.headerInner}>
					...
					<div className={styles.headerActions}>
						{/* BUG-001 FIX: navega para /entrar */}
						<button className={styles.headerBtn} onClick={() => navigate("/entrar")}>
							<FiUser size={16} /> Login / Registro
						</button>
						{/* CARRINHO FIX: navega para /carrinho e exibe badge */}
						<button className={styles.headerBtn} onClick={() => navigate("/carrinho")} style={{ position: "relative" }}>
							<FiShoppingCart size={16} /> Carrinho
							{qtdTotal > 0 && (
								<span className={styles.carrinhoBadge}>{qtdTotal}</span>
							)}
						</button>
					</div>
				</div>
				<nav className={styles.nav}>
					{["Eletrônicos", "Vestuários", "Licenças", "E-books", "Saúde"].map(cat => (
						<button key={cat} className={styles.navItem} onClick={() => navigate(`/categoria/${encodeURIComponent(cat)}`)}>{cat} <FiChevronDown size={12} /></button>
					))}
				</nav>
			</header>
				...
				{buscando ? (
					<section className={styles.gridSection}>
						<div className={styles.gridHeader}>
							<h3>Resultados para <span className={styles.gridCat} style={{ color: corSecundaria }}>"{busca}"</span></h3>
							<button className={styles.vejaBtn} onClick={() => setBusca("")}>Limpar busca</button>
						</div>
					...
					<>
						...
						<section className={styles.heroSection}>
							<div className={styles.heroLeft} style={{ background: corPrimaria }}>
								...
								<button className={styles.heroBtn} onClick={() => navigate(`/categoria/${encodeURIComponent(grid1.categoria)}`)}>Compre já! </button>
							...
							<div className={styles.heroRight} style={{ background: corSecundaria }}>
								...
								<button className={styles.heroBtn} onClick={() => navigate(`/categoria/${encodeURIComponent(grid2.categoria)}`)}>Compre já!</button>
								<div className={styles.heroImgWrap}>
									{banner2.imagem
						...
						<section className={styles.gridSection}>
							<div className={styles.gridHeader}>
								<h3>
									{grid1.titulo.replace("{categoria}", "")}
									<span
										className={styles.gridCat}
										style={{ color: corSecundaria, cursor: "pointer", textDecoration: "underline" }}
										onClick={() => navigate(`/categoria/${encodeURIComponent(grid1.categoria)}`)}
									>
										{grid1.categoria}
									</span>
								</h3>
								<button className={styles.vejaBtn} onClick={() => navigate(`/categoria/${encodeURIComponent(grid1.categoria)}`)}>Veja mais <FiChevronRight size={14} /></button>
							</div>
							<div className={styles.gridLine} style={{ background: corPrimaria }} />
							<div className={styles.prodGrid}>
								{prodGrid1.length > 0
						...
						<section className={styles.gridSection}>
							<div className={styles.gridHeader}>
								<h3>{grid2.titulo.replace("{categoria}", "")} <span
									className={styles.gridCat}
									style={{ color: corSecundaria, cursor: "pointer", textDecoration: "underline" }}
									onClick={() => navigate(`/categoria/${encodeURIComponent(grid2.categoria)}`)}
								>
									{grid2.categoria}
								</span></h3>
								<button className={styles.vejaBtn} onClick={() => navigate(`/categoria/${encodeURIComponent(grid2.categoria)}`)}>Veja mais <FiChevronRight size={14} /></button>
							</div>
							<div className={styles.gridLine} style={{ background: corPrimaria }} />
							<div className={styles.prodGrid}>
								{prodGrid2.length > 0
						...
						<section className={styles.gridSection}>
							<div className={styles.gridHeader}>
								<h3>{grid3.titulo.replace("{categoria}", "")}
									<span
										className={styles.gridCat}
										style={{ color: corSecundaria, cursor: "pointer", textDecoration: "underline" }}
										onClick={() => navigate(`/categoria/${encodeURIComponent(grid3.categoria)}`)}
									>
										{grid3.categoria}
									</span>
								</h3>
								<button className={styles.vejaBtn} onClick={() => navigate(`/categoria/${encodeURIComponent(grid3.categoria)}`)}>
									Veja mais <FiChevronRight size={14} />
								</button>
							</div>
							<div className={styles.gridLine} style={{ background: corPrimaria }} />
							<div className={styles.prodGrid}>
					...
					<div className={styles.footerLinks}>
						<div>
							...
							{["Eletrônicos", "Saúde", "Vestuário", "E-books", "Cuidado pessoal", "Artesanato"].map(c => (
								<p key={c} className={styles.footerLink} onClick={() => navigate(`/categoria/${encodeURIComponent(c)}`)} style={{ cursor: "pointer" }}>· {c}</p>
							))}
						</div>
						<div>
